...
export default function AgentsPage() {
  ...
  const agentTasks = DEMO_TASKS.filter(t => t.createdByAgent);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="text-center"><p className="text-2xl font-bold text-neon-400">{agentTasks.filter(t => t.status === 'open').length}</p><p className="text-xs text-surface-500">active tasks</p></Card>
        ...
        <Card glow className="!border-neon-500/20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <pre className="bg-surface-950 border border-white/[0.08] rounded-lg p-3 font-mono text-xs text-surface-300">{`{
  "mcpServers": {
    "openhuman": {
      "command": "npx",
      "args": ["openhuman-mcp"],
        ...
        <div className="space-y-3">
          {CONNECTED_AGENTS.map(a => (
            <Card key={a.id} className="flex items-center gap-4">
              <div className="h-10 w-10 rounded-lg bg-white/[0.06] flex items-center justify-center text-lg border border-white/[0.08]">🤖</div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-semibold text-white">{a.name}</h3>
                  <Badge variant="secondary" className="text-xs">{a.type}</Badge>
                  <Badge className={`text-xs ${a.status === 'active' ? 'bg-neon-500/15 text-neon-400 border-neon-500/30' : 'bg-white/[0.06] text-surface-400 border-white/[0.08]'}`}>{a.status}</Badge>
                </div>
        ...
        <div className="space-y-2">
          {agentTasks.slice(0, 6).map(task => (
            <Card key={task.id} className="!p-3 flex items-center gap-3">
              <Badge className={`text-xs flex-shrink-0 ${task.status === 'open' ? 'bg-blue-500/15 text-blue-400 border-blue-500/30' : task.status === 'completed' ? 'bg-neon-500/15 text-neon-400 border-neon-500/30' : 'bg-yellow-500/15 text-yellow-400 border-yellow-500/30'}`}>{task.status}</Badge>
